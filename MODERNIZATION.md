...
|---|---|---|
| **0 · Foundations** ✅ | Gradle multi-module; GitHub Actions running `SimTests` on every push; desktop app still launches | `gradle check` runs the suite in CI; no behaviour change |
...
| **2 · Backend service** ✅ | Javalin app: create world from seed, WS state stream **and command intake**, baked static-layer endpoints, health; Dockerfile; a throwaway debug page as living proof | Watch deltas over WS, inject a `SpawnItem` command, see it appear in the stream — verified end-to-end in a real (phone-sized) browser |
| **3 · Web client v1** ✅ | Vite/TS canvas viewer: baked layers under 10 Hz-interpolated entities; unified pointer pan + pinch/wheel zoom; tap-to-follow (camera glued to a creature); **action UI: spawn food/crate/hazard at a tapped tile, pause/resume/speed**; token-gated commands, auto-reconnect; responsive + `image-rendering: pixelated`. Built by Gradle/Docker into the server jar; CI compiles it | **Watch — and feed — a live world from a phone**; verified via Playwright (phone viewport): zoom crisp, follow tracks to 0.08 tiles, tap-spawn appears in-stream |
| **4 · Visual parity + inspect** ✅ | Server-baked creature sprite atlases (real ProcCreature organisms, 8 dirs × 8 idle frames per phenotype, fliers included); pheromone haze, corpses, items, carry/grab rings; tap an entity → info panel (genome, energy, state); minimap with viewport box + tap-to-jump; a small-species demo herd | Web view reads like the Swing view — organisms not dots; verified via Playwright |
| **5 · Public URL + ops** ✅ | Deployed on a Hetzner VPS behind a Cloudflare-fronted TLS edge; command channel token-gated (view open, meddling authenticated); metrics (`/api/metrics` JSON + `/metrics` Prometheus: tick ms, entities, viewers, heap, uptime); replay — download a session as a `seed + command log` recording and reconstruct the world at any tick (`/api/world/recording`, `/api/replay`), durably persisted to a volume; auto-deploy on push to master | A live public URL; a long-running world to check on over morning coffee |
