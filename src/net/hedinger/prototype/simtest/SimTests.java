...
public class SimTests {
	...
	static class LethalDamageAndScavenging extends Scenario {
		...
		public void run() {
			...
			// turned, then the 100-tick deathspan runs.
			for (int t = 0; t < 2000 && h.freshMeat() > 0; t++) {
				tick(w, 1);
			}
			assertTrue("the body has turned and still lies there", !h.isRemoved());
			tick(w, 50); // partway through the 100-tick deathspan
			assertTrue("corpse lingers while within its deathspan", !h.isRemoved());
			tick(w, 80); // 130 ticks of decay, past the deathspan
	...
	static class CorpseRotsForAsLongAsItTookToGrow extends Scenario {
		...
		public void run() {
			...
			// Rot begins when the fresh meat has turned; the span is measured from there.
			for (int t = 0; t < 2000 && b.freshMeat() > 0; t++) {
				tick(w, 1);
			}
			assertLess("and still barely decayed the moment it turns", b.decayProgress(), 0.05);
	...
	/**
	 * A scavenger eats carrion, and what that does to the corpse is exactly
	 * this: it takes fresh meat with the flesh, so the body turns sooner and its
	 * rot begins earlier -- and nothing else. Once both are rotting they rot at
	 * the same pace, because decay is time and never eating. Pinned against a
	...
	static class ScavengerEatsCarrionButDoesNotRotIt extends Scenario {
		...
		public void run() {
			...
			assertTrue("both have turned", body.freshMeat() == 0 && alone.freshMeat() == 0);
			assertTrue("the eaten one began rotting no later than the one left alone",
					body.decayProgress() >= alone.decayProgress() - 1e-9);
			// From here the clocks run at the same pace: decay is time, never eating.
			double b0 = body.decayProgress(), a0 = alone.decayProgress();
			tick(w, 50);
			tick(w2, 50);
			assertNear("but once rotting, an eaten body rots at exactly the pace of one left alone",
					alone.decayProgress() - a0, body.decayProgress() - b0, 0.001);
		}
	}
	 ...
	 */
	static class AFreshCarcassSitsBeforeItRots extends Scenario {
		...
		public void run() {
			...
			int half = NPC.FRESH_TICKS / 2;
			tick(w, half);
			assertGreater("halfway through the window most of it is still fresh -- slowly first",
					ref.freshMeat(), 0.7 * freshAtDeath);
			assertNear("what has spoiled so far is decayed meat now, not gone",
			...
			// --- who eats what: three mouths on one fresh body, and a control.
			World m = barren();
			TestNPC prey = TestNPC.grazer(10.5, 6.5, 0, body(12)).grown();
			TestNPC control = TestNPC.grazer(3.5, 3.5, 0, body(12)).grown(); // out of every reach
			m.spawnEntity(prey);
			m.spawnEntity(control);
			tick(m, 1);
			prey.kill();
			control.kill();
			tick(m, 1);
			...
			m.spawnEntity(hunter);
			m.spawnEntity(scav);
			m.spawnEntity(para);
			tick(m, 1);
			double h0 = hunter.totalSwallowed(), s0 = scav.totalSwallowed(), p0 = para.totalSwallowed();
			tick(m, 20);
			assertGreater("a hunter eats fresh meat", hunter.totalSwallowed() - h0, 0);
			assertGreater("a scavenger eats it too, fresh or not", scav.totalSwallowed() - s0, 0);
			assertTrue("a parasite lives off the living and touches no corpse",
					para.totalSwallowed() - p0 < 1e-9);
			assertLess("every bite took fresh meat with it, so the eaten body turns sooner",
					prey.freshMeat(), control.freshMeat());
			// Let it turn, then the hunter is done with it and the scavenger is not.
			for (int t = 0; t < 600 && prey.freshMeat() > 0; t++) {
				tick(m, 1);
			}
			...
			assertTrue("eaten to nothing, it still lies there", prey.meatLeft() <= 0.01 && !prey.isRemoved());
			tick(m, prey.getDeathspan() + 5);
			assertTrue("and dissolves into the ground when its own clock runs out", prey.isRemoved());
		}
	}

	/**
	 * Decayed meat rots on the clock. Once a body has turned, the scavengers'
	 * meat thins along {@code 1 - p^ROT_SHAPE} of the decay progress -- slowly at
	 * first, then fast -- and reaches nothing exactly as the corpse dissolves, so
	 * a late scavenger finds less and the last of it feeds the ground. A
	 * scavenger's bite takes some and leaves less to rot; it does not hurry the
	 * clock: two bodies, one bitten and one not, dissolve on the same tick. And
	 * the ground is paid only for what no mouth took.
	 */
	static class DecayedMeatRotsOnTheClock extends Scenario {
		@Override
		public void run() {
			seed(51);
			World w = room(14, 10);
			...
			TestNPC whole = TestNPC.inert(3.5, 4.5, 0).withSize(8).withDeathspan(400);
			TestNPC bitten = TestNPC.inert(10.5, 4.5, 0).withSize(8).withDeathspan(400);
			w.spawnEntity(whole);
			...
			double p0 = whole.decayProgress();
			assertNear("turned, two thirds of the body is decayed meat",
					(NPC.FRESH_SHARE + (1 - NPC.FRESH_SHARE) * NPC.SCAVENGER_SHARE) * mass, d0, 0.02);
			assertNear("and the two bodies are alike", d0, bitten.decayedMeat(), 1e-9);
			...
			for (int t = 0; t < 1000 && whole.decayProgress() < 0.5; t++) {
				tick(w, 1);
			}
			double p = whole.decayProgress();
			double curve = (1 - Math.pow(p, NPC.ROT_SHAPE)) / (1 - Math.pow(p0, NPC.ROT_SHAPE));
			assertNear("halfway through the clock the meat has thinned along the curve",
					d0 * curve, whole.decayedMeat(), 0.03 * d0);
			assertGreater("most of it is still there: slowly first", whole.decayedMeat(), 0.6 * d0);
			assertLess("but less than at the turn", whole.decayedMeat(), 0.9 * d0);
			assertNear("the bitten body rots at the same pace, on what it has left",
					(d0 - bite) * curve, bitten.decayedMeat(), 0.03 * d0);
			assertNear("what lies there is the meat still on it plus the bones",
					whole.decayedMeat() + whole.bones(), whole.remainingMass(), 1e-9);
			assertLess("so the body is lighter than it was", whole.remainingMass(), 0.9 * mass);

			double groundWhole = w.getTile(3, 4, 0).getFertility();
			double groundBitten = w.getTile(10, 4, 0).getFertility();
			for (int t = 0; t < 1000 && !whole.isRemoved() && !bitten.isRemoved(); t++) {
				tick(w, 1);
			}
			assertTrue("the untouched body has dissolved", whole.isRemoved());
			assertTrue("and the bitten one on the same tick: a bite does not hurry the clock", bitten.isRemoved());
			assertLess("with nothing left to rot when it went", whole.decayedMeat(), 1e-6);
			double giftWhole = w.getTile(3, 4, 0).getFertility() - groundWhole;
			double giftBitten = w.getTile(10, 4, 0).getFertility() - groundBitten;
			assertGreater("the ground was fed by what rotted and the bones", giftWhole, 0.05);
			assertNear("and the bitten body fed it less by exactly the mouthful taken",
					giftWhole * (mass - bite) / mass, giftBitten, 0.002);
		}
	}
	...
	static class FatIsTheBodysStore extends Scenario {
		...
		public void run() {
			...
			b.spawnEntity(rich);
			tick(b, 1);
			double structure = rich.leanMass(), stored = rich.fat();
			rich.kill();
			tick(b, 1);
			assertNear("the carcass weighs lean mass and fat together", structure + stored, rich.carcassMass(), 1e-9);
			assertNear("half the fat is fresh meat, over the lean third",
					NPC.FRESH_SHARE * structure + stored / 2, rich.freshMeat(), 0.01);
			assertNear("half is decayed meat, over the lean share",
	 ...
	 * meat is fresh. Before, a corpse was never quarry: the hunter that made
	 * the kill ate what it could and every other hunter walked past.
	 */
	static class OtherHuntersJoinTheKill extends Scenario {
		private static Genome body(double size) {
			Genome g = new Genome();
			g.size = size;
			g.speed = 0.0005;
		...
		public void run() {
			...
			// A parked animal and the hunter that will bring it down.
			TestNPC prey = TestNPC.grazer(8.5, 6.5, 0, body(12)).grown();
			TestNPC killer = TestNPC.predator(8.9, 6.5, 0, body(16)).grown().withHunger(1.0)
					.withReproCooldown(100_000_000);
			w.spawnEntity(prey);
			w.spawnEntity(killer);
			// A second, hungry hunter with the scene in plain sight but well out
			// of reach, driven by the ordinary hunter seed: it seeks forage, which for
			// a hunter is its prey scan. Nothing living is left for it to hunt, so if
			// its forage channel lights up, what lit it is the carcass.
			Genome jg = body(16);
			jg.speed = 0.06;
			jg.brain = net.hedinger.prototype.sim.Worlds.hunterBrain();
			TestNPC joiner = TestNPC.mindedPredator(15.5, 6.5, 0, jg).grown().withHunger(1.0)
					.withReproCooldown(100_000_000);
			w.spawnEntity(joiner);
			double j0 = joiner.totalSwallowed();
			...
			double before = prey.meatLeft();
			for (int t = 0; t < NPC.FRESH_TICKS + 200 && prey.meatLeft() >= before - 0.05; t++) {
				tick(w, 1);
			}
			assertGreater("a second hunter came to the fresh carcass and ate off it",
					joiner.totalSwallowed() - j0, 0.0);
			assertLess("the meat it took came off the body", prey.meatLeft(), before);
		}
	}
	...
	static class ACarcassIsAMealForSeveral extends Scenario {
		...
		public void run() {
			...
			// A fat body of the same lean mass is that much more of a meal.
			TestNPC rich = TestNPC.grazer(6.5, 6.5, 0, g).grown().fattened();
			w.spawnEntity(rich);
			tick(w, 2);
			rich.kill();
			tick(w, 1);
			assertGreater("a fat body carries more meat than its lean mass", rich.edibleMass(), edible + 0.2);
	...
	static class ACorpseLingersByItsMass extends Scenario {
		...
		private int corpseTicks(double size) {
			...
			for (int i = 1; i <= 4000; i++) { // fresh window, then the span
				tick(w, 1);
				if (t.isRemoved()) {
					return i;
				}
		...
		private double[] hunt(boolean withPrey) {
			...
			if (withPrey) {
				...
				for (int round = 0; round < 4; round++) {
					...
					for (int t = 0; t < 500 && prey.freshMeat() > 0 && prey.meatLeft() > 0.01 && !prey.isRemoved(); t++) {
						tick(w, 1);
					}
					prey.remove();
				}
	 ...
	 * lineage rather than a constant worth choosing here.
	 *
	 * <p>Same world twice, same brain, same everything but the gene. The hunter
	 * is set walking at a body across the field and then a second, better one is
	 * put in front of it: an opportunist turns and takes the newcomer, and a
	 * dogged one finishes what it was already running down. Which body dies first
	 * is the whole measurement.
	 *
	 * <p>The newcomer is placed off to one side rather than on the path, because a
	 * body the hunter merely walks INTO gets bitten whatever its mind wanted --
	 * the bite takes what is nearest on arrival. Putting it beside the route is
	 * what keeps this a test of the choice rather than of the geometry.
	 */
	static class DoggednessIsALineagesOwnBusiness extends Scenario {
		private static Brain hunterBrain() {
			int[][] p = new int[12][];
			p[0] = new int[] { Brain.SET, 0, 8, 0 }; // r0 = 0.5 -> SEEK_PREY
			p[1] = new int[] { Brain.WRITE, AgentIO.A_SEEK, 0, 0 };
			p[2] = new int[] { Brain.SET, 1, 9, 0 }; // r1 = 1.0
		...
		private String outcome(double determination) {
			...
			hunter.withHeading(ahead);
			double cx = Math.cos(ahead), cy = Math.sin(ahead);
			TestNPC first = TestNPC.grazer(hx + cx * 9, hy + cy * 9, 0, body(12, 0.0005, null));
			w.spawnEntity(first);
			tick(w, 60); // committed and under way, but nowhere near arriving

			// Better by every standard -- same size, a third of the walk -- and well
			// off the line of march so it cannot simply be bumped into.
			double nx = hunter.getX() + cx * 1.5 - cy * 1.5;
			double ny = hunter.getY() + cy * 1.5 + cx * 1.5;
			TestNPC newcomer = TestNPC.grazer(nx, ny, 0, body(12, 0.0005, null));
			w.spawnEntity(newcomer);

			for (int t = 0; t < 12000 && !first.isDead() && !newcomer.isDead(); t++) {
				tick(w, 1);
			}
			return first.isDead() ? "first" : newcomer.isDead() ? "newcomer" : "neither";
		}

		@Override
		public void run() {
			assertTrue("an opportunist turns for the better body",
	...
	static class AFounderHunterChasesFlatOut extends Scenario {
		...
		public void run() {
			...
			assertLess("and it is clear of starving", hunter.getHunger(), TestNPC.STARVE_HUNGER);
	 ...
	 * transaction hands a child enough to finish growing rather than enough to
	 * start.
	 *
	 * <p>Measured over a COHORT, not one child. Whether any single juvenile
	 * lands its kills is a lottery: following the firstborn alone, this
	 * scenario passed on two seeds in six before the endowment was rewritten
	 * and three in six after, which is a test of luck rather than of the
	...
	static class BrainMatesViaActuator extends Scenario {
		...
		public void run() {
			...
			// Sixteen founders, not eight: whether any ONE pairing yields a mind
			// carrying both signatures is a coin flip (Brain.child mixes per slot),
			// so the scenario needs enough matings for the crossover it asserts to
			// be a property of sexual reproduction rather than of the seed. At eight
			// it passed on five seeds in eight, which is a test of luck.
			World w = room(20, 12);
			for (int i = 0; i < 16; i++) {
				Genome g = new Genome();
	...
	static class ABodyIsEatenOnce extends Scenario {
		...
		public void run() {
			...
			double paidAlive = 0;
			for (int t = 0; t < 3000 && !prey.isDead(); t++) {
				paidAlive = hunter.totalSwallowed(); // what it had swallowed with the animal still alive
				tick(w, 1);
			}
	 ...
	 * animal is dead, and the animal that shakes it off mends for free, because
	 * nothing was taken.
	 *
	...
	static class MendingBuysBackTheFlesh extends Scenario {
		...
		public void run() {
			...
			TestNPC drained = grazer(22.5, 10.5);
			TestNPC control = grazer(22.5, 8.5);
			w.spawnEntity(bitten);
			w.spawnEntity(drained);
			w.spawnEntity(control);
			...
			assertEquals("the drain has closed", 100, drained.getHealth());
			assertEquals("the control never had a wound", 100, control.getHealth());
	 ...
	 * better-funded child than a budder of the same lineage, which is the
	 * whole advantage of mating; a parent whose price has drifted above what
	 * it holds pays only what it has; and a parent that cannot afford the
	...
	static class NoFreeEnergyAtBirth extends Scenario {
		 ...
		 *  parents held AND what they were offering at the start of every tick,
		 *  so both readings are the ones from the birth tick itself. Returns
		 *  null if none arrived. */
		private static TestNPC breedOut(Scenario s, World w, double[] before, double[] offer,
				TestNPC... parents) {
			for (int t = 0; t < 4000; t++) {
				double sum = 0, bid = 0;
				for (TestNPC p : parents) {
					sum += held(p);
					bid += p.birthPayment();
				}
				before[0] = sum;
		...
		public void run() {
			seed(102);
			// One tick of the parents' own living drifts the books between the
			// reading and the birth; the burn is well under a hundredth.
			double eps = 0.03;
			...
			m.spawnEntity(pb);
			m.think();
			double[] beforeKid = new double[1], bidKid = new double[1];
			TestNPC kid = breedOut(this, m, beforeKid, bidKid, pa, pb);
			assertTrue("the pair bred", kid != null);
			double pairSpent = beforeKid[0] - held(pa) - held(pb);
			assertNear("the child is worth exactly what BOTH parents gave up ("
			...
			double poorSpent = beforeCheap[0] - held(poor);
	...
	static class CorpseFeedsTheGround extends Scenario {
		...
		public void run() {
			...
			tick(w, 2);
			victim.kill();
			for (int t = 0; t < 2000 && victim.freshMeat() > 0; t++) {
				tick(w, 1); // freshly dead: the decay clock waits
			}
